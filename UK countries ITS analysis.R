...
testAutocorr <- function(data, model, max.lag=10) {
...
{print(ggplot(., aes(Year, Value, group=interaction(Country, Cat2), col=Country))+
  ...
  lm(Value ~ Time + England + Time_Eng + Cat2 + Trend2 + Cat2_Eng + Trend2_Eng, data=.) %>% 
...
modScot99_07_null <- gls(Value ~ Time + 
                           England + 
                           Time_Eng + 
                           Cat1 + 
                           Trend1 + 
...
modScot99_07_null_cfac <- EngScotContro[8:25,] %>% 
  ...
  mutate(Cat1_Eng = c(rep(0,9), rep(1,9)),
...
EngScotContro %>%
  ...
  ggplot(aes(Time, Value, col = Country, group = interaction(Country, Cat1, Cat2))) +
  geom_point() +
  geom_smooth(  # predicted values and 95% Confidence Interval
...
EngScotContro %>% 
  filter(Year<1999|Year>2007) %T>% 
{print(ggplot(., aes(Year, Value, group=interaction(Country, Cat1, Cat2), col=Country))+
         geom_point() + geom_smooth(method="lm", se=FALSE))} %>% 
  lm(Value ~ Time + England + Time_Eng + Cat2 + Trend2 + Cat2_Eng + Trend2_Eng, data=.) %>% 
  assign("modScotPI",., envir = .GlobalEnv) %>% 
  summary()

testAutocorr(EngScotContro, modScotPI, 14)  # Plotting residuals (top) invalid - data not filtered
                                            # Possibly no autocorrelation? Sig at PACF 12 and 13

modScotPI_null <- gls(Value ~ Time + England + Time_Eng + Cat2 + Trend2 + Cat2_Eng + Trend2_Eng,
...
modScotPI_null_cfac <- tibble(Time=c(17:25),
                            England=1,
                            Cat2=1,
                            Trend2=c(1:9),
                            Time_Eng=17:25,
                            Cat2_Eng=0,
                            Trend2_Eng=0)  # Emlulating England with no added effect
