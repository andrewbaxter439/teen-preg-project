...
tb_u18_sp <- super_md_u18_sp %>% 
  ...
  geom_segment(aes(x = IntYr, xend = IntYr, y = 0, yend = 20), linetype = "dotted", size = 1, col = "grey", alpha = 0.7) +
  geom_line(size = 1.5) +
  theme_sphsu_light() +
...
synthPrep(synthData_u18[,1:5] %>% filter(!Country %in% exclude_u18_gdp_1990),
          ...
          special.predictors = list(
            list("rate", 1990:1994, "mean"),
            list("rate", 1995:1998, "mean")
          ),
          time.optimise.ssr = opt_y1:1998,
...
# ggsave("graphs/Synth-u18-gdp.svg", width = 200, height = 150, units = "mm")
...
st_u18_gdp$tab.w %>% arrange(desc(w.weights)) %>% select(Country = unit.names, Weight = w.weights) %>% 
  kable()
st_u18_gdp$tab.v %>% kable()
```

### Placebo testing by country and time
```{r plot_u18_gdp_pb_nat}
# gaps graph
pb_plot <- gg_gaps(md_u18_gdp, pl_u18_gdp) +
...
# ggsave("graphs/Synth-u18-all.svg", width = 200, height = 150, units = "mm")
...
tp_u20_sp_plotly <- super_md_u20_sp %>% 
  ggplot(aes(Year, Rate, col = Group, linetype = Group, frame = IntYr)) +
  geom_segment(aes(x = IntYr, xend = IntYr, y = 0, yend = 80), linetype = "dotted", size = 1, col = "grey", alpha = 0.7) +
  geom_line(size = 1.5) +
  theme_sphsu_light() +
