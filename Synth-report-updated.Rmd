...
if (params$save_graphs){ggsave(plot = pp_u18_sp, "graphs/Synth-u18-sp-pp-ratio.svg", width = 200, height = 150, units = "mm")}
 if (params$save_graphs){ggsave(plot = pb_plot_u18_sp, "graphs/Synth-u18-sp-gaps.svg", width = 200, height = 150, units = "mm")}
```
...
```{r md_u18_ns_graph}
gr_u18_ns <- gg_synth(md = md_u18_ns, post = FALSE) + xlim(1990, 2013)
...
```{r plot_u18_pb_ns, include = FALSE}
pb_plot_u18_ns <- 
...
```{r u18_pb_time_graph_ns, fig.show="animate",	interval = 1/20, include = FALSE}
...
```{r md_u20_ns_graph}
# graph
gr_u20_ns <- gg_synth(md = md_u20_ns, post = FALSE) + xlim(1990, 2013) 
...
### Placebo testing by country and time
```{r plot_u20_ns_pb_nat, include = FALSE}
# gaps graph
pb_plot_u20_ns <- 
...
synthPrep(sd_noScot,
          "u18_all",
          dependent = "rate",
          special.predictors = append(sp_u18_filt, sp_u20_gdp) %>% 
            append(sp_u20_edu) %>% 
            append(sp_u20_mob) %>% 
            append(sp_u20_urb),
          time.optimise.ssr = opt_y1:1998,
...
```{r md_u18_all_graph}
# graph
gr_u18_all <- gg_synth(md = md_u18_all, post = FALSE)  + xlim(1990, 2013)
...
synthPrep(sd_noScot,
          "u20_all",
          dependent = "pRate",
          special.predictors = append(sp_u20_filt, sp_u20_gdp) %>% 
            append(sp_u20_edu) %>% 
            append(sp_u20_mob) %>% 
            append(sp_u20_urb),
